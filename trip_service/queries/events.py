...
class EventRepository(Queries):
    ...
    def get_all_events(self, account_id, trip_id) ->List[EventOut]:
        result = self.collection.find({"account_id": account_id, "trip_id": trip_id})
        new_list = []
        for event in result:
    ...
    def get_event(self, event_id, trip_id) -> EventOut:
        event_dict = self.collection.find_one({"_id": ObjectId(event_id), "trip_id": trip_id})
        if event_dict == None:
            raise HTTPException(status_code=404, detail="event_id not found")
        event_dict["id"] = event_id
        return EventOut(**event_dict)

    def update_event(self, event: EventIn, event_id: str, account_id: str, trip_id: str) -> EventOut:
        props = event.dict()
        props['account_id'] = account_id
        self.collection.update_one({"_id": ObjectId(event_id), "trip_id": trip_id}, {"$set": props})
        props['id'] = event_id
        props['trip_id'] = trip_id
        return EventOut(**props)
    ...
    def delete_event(self, event_id, trip_id):
        ...
        if event_dict == None:
            return False
        response = self.collection.delete_one({"_id": ObjectId(event_id), "trip_id": trip_id})
        return True
