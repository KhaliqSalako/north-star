...
function TripListCard({ trip, getTripData, name }) {
  ...
  const deleteTrip = async (trip_id) => {
    await fetch(`${process.env.REACT_APP_ACCOUNTS_HOST}/api/trips/${trip_id}`, {
      method: "DELETE",
      credentials: "include",
      mode: "cors",
      headers: {
  ...
  return (
    ...
    >
      ...
      <div
        className="card-body w-100 h-25 d-flex justify-content-center align-items-center"
        style={{ position: "absolute", top: "300px" }}
      >
        <div className="">
          <h1 className="text-white pt-2 text-wrap custom-font text-shadow">
            {trip.name}
          </h1>
        </div>
      </div>
      <p
        className="card-text text-white fw-bold text-shadow w-100"
        style={{ position: "absolute", bottom: "35px" }}
      >
        {wordDateStart} - {wordDateEnd}
      </p>
      <div
        className="row-cols-2 align-content-end w-100"
